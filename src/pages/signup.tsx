...
export const Signup = () => {
    ...
    const handleDuplicateIdCheckClick = async () => {
        try {
            const res = await axios.post<{ result: boolean }>(
                `${process.env.VITE_SOCKET_SERVER}/check/id`,
                { id: data.id }
            );
            ...
            dispatch(setOpenModal("notified"));
            dispatch(setContent("서버에 문제가 발생하였습니다"));
        }
    };
    ...
    const handleDuplicateEmailCheckClick = async () => {
        try {
            const res = await axios.post<{ result: boolean }>(
                `${process.env.VITE_SOCKET_SERVER}/check/email`,
                { id: data.email }
            );
